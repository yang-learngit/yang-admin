...
import {baseURL, ajax, upload, download, staticDownload} from './ajax'
...
util.setToken = (token) => {
  Cookies.set(util.TOKEN_KEY, token, {expires: config.cookieExpires || 1})
}

util.getToken = () => {
  const token = Cookies.get(util.TOKEN_KEY)
  if (token) return token
  else return false
}
 ...
 * @returns {Array}
 */
util.getBreadCrumbList = (routeMetched, homeRoute) => {
  let res = routeMetched.filter(item => {
    return item.meta === undefined || !item.meta.hide
  }).map(item => {
    let obj = {
  ...
  res = res.filter(item => {
    return !item.meta.hideInBreadCrum
  })
  return [Object.assign(homeRoute, {to: homeRoute.path}), ...res]
}

util.showTitle = (item, vm) => vm.$config.useI18n ? vm.$t(item.name) : ((item.meta && item.meta.title) || item.name)
...
util.getHomeRoute = routers => {
  ...
  while (++i < len) {
    let item = routers[i]
    if (item.children && item.children.length) {
      let res = util.getHomeRoute(item.children)
      if (res.name) return res
    } else {
      if (item.name === 'home') homeRoute = item
    }
...
util.base32 = function (srcStr) {
  ...
  for (let i = 0; i < srcStr.length;) {
    // let          index    = 0;
    currByte = (srcStr.charCodeAt(i) >= 0) ? srcStr.charCodeAt(i) : (srcStr.charCodeAt(i) + 256)
    if (index > 3) {
      if ((i + 1) < srcStr.length) {
        nextByte = (srcStr.charCodeAt(i + 1) >= 0)
          ? srcStr.charCodeAt(i + 1)
          : (srcStr.charCodeAt(i + 1) + 256)
      } else {
        nextByte = 0
      }
  ...
  if (retrunString.length < 16) {
    retrunString = (retrunString + '==========').substring(0, 16)
  }

  return retrunString
}

util.httpQueryString = function (o) {
  let queryString = ''
  for (let key in o) {
...
util.render = function (h, data) {
  return h('span', data.column.key.split('.').reduce(function (obj, key) {

...
import { menuRouters, otherRouters } from '@/router/routers'

export default {
  state: {
    breadCrumbList: [],
    tagNavList: [],
    homeRoute: util.getHomeRoute(otherRouters)
  },
  getters: {
    menuList: (state, getters, rootState) => util.getMenuByRouter(menuRouters, rootState.user.access)
  },
  mutations: {
    setBreadCrumb (state, routeMetched) {
    ...
    addTag (state, { route, type = 'unshift' }) {
      if (!util.routeHasExist(state.tagNavList, route)) {
        if (type === 'push') {
